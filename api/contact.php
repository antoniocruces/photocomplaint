...
define("NAMEFROM", "Contacto Fotodenuncia / PhotoComplaint Contact");
...
define("MSG", [
	"en" => [
		...
		"accept" => "accept",
		"images" => "images",
		"subject" => "PhotoComplaint Contact"
	],
	"es" => [
		"language" => "idioma",
		"date" => "fecha",
...
function build_message($ticket) {
	...
	$content []= '<tr><td>' . $t['type'] . '</td><td>' . htmlspecialchars($_POST['type']) . '</td></tr>';
	$content []= '<tr><td>' . $t['id'] . '</td><td>' . htmlspecialchars($_POST['id']) . '</td></tr>';
...
$names = empty($_FILES['files']['name']) ? array() : $_FILES['files']['name'];
...
$response = array("status" => $status, "ticket" => $ticket);
